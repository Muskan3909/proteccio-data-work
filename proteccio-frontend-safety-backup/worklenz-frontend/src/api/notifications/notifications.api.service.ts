...
export const notificationsApiService = {
  getNotifications: async (filter: string): Promise<IServerResponse<IProteccioNotification[]>> => {
    const q = toQueryString({ filter });
    const response = await apiClient.get<IServerResponse<IProteccioNotification[]>>(
      `${rootUrl}${q}`
    );
