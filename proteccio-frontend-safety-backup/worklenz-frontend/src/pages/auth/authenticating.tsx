...
const AuthenticatingPage: React.FC = () => {
  ...
  const handleSuccessRedirect = () => {
    const project = localStorage.getItem(Proteccio_REDIRECT_PROJ_KEY);
    if (project) {
      localStorage.removeItem(Proteccio_REDIRECT_PROJ_KEY);
      window.location.href = `/Proteccio/projects/${project}?tab=tasks-list`;
      return;
    }
    ...
    const handleAuthentication = async () => {
      try {
        ...
        if (!session.user.setup_completed) {
          return navigate('/Proteccio/setup');
        }
