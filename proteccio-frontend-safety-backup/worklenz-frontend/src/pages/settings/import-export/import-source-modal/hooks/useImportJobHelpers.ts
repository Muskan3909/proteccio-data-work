...
interface UseImportJobHelpersArgs {
  ...
  ProteccioStatuses: IProjectStatus[];
  setProteccioStatuses: React.Dispatch<React.SetStateAction<IProjectStatus[]>>;
...
export const useImportJobHelpers = ({
  ...
  const ensureDefaultProjectStatusId = React.useCallback(async (): Promise<string> => {
    ...
    if (ProteccioStatuses.length) {
      const defaultStatus = pickDefault(ProteccioStatuses);
      if (defaultStatus?.id) {
