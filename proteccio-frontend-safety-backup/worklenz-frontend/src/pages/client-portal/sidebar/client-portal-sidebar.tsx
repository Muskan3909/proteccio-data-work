...
const ClientPortalSidebar: React.FC<ClientPortalSidebarProps> = ({
    ...
    () =>
      menuSource.map(item => ({
        ...
        label: collapsed ? null : (
          <Link to={`/Proteccio/client-portal/${item.endpoint}`}>
        ...
        onClick: collapsed
          ? () => {
              ...
              navigate(`/Proteccio/client-portal/${item.endpoint}`);
            }
          : () => {
  ...
  const activeKey = useMemo(() => {
    const afterProteccioString = location.pathname?.split('/Proteccio/client-portal/')[1];
    return afterProteccioString?.split('/')[0];
  }, [location.pathname]);
