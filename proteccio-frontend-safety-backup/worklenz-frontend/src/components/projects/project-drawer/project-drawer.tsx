  ...
  const handleFormSubmit = async (values: any) => {
    try {
      ...
      if (response?.data?.done) {
        const savedProjectId = editMode ? projectId : response.data.body?.id;
        if (savedProjectId && canManageBudgetSettings) {
          ...
          if (!editMode || selectedBudget !== currentBudget) {
            await projectFinanceApiService.updateProjectBudget(savedProjectId, selectedBudget);
          }

          if (!editMode || selectedCurrency !== currentCurrency) {
            await projectFinanceApiService.updateProjectCurrency(savedProjectId, selectedCurrency);
          }
        }
      ...
      children: (
        <>
          ...
          <ProjectHealthSection
            ...
            form={form}
            t={t}
            disabled={!isProjectManager && !isOwnerorAdmin && !isCurrentUserTeamLead(currentSession)}
          />
      ...
      children: (
        <>
          ...
          <Form.Item
            ...
            valuePropName="checked"
          >
            <Switch disabled={!isProjectManager && !isOwnerorAdmin && !isCurrentUserTeamLead(currentSession)} />
          </Form.Item>
        </>
      ),
    },
        ...
        <>
          ...
          >
            <Input
              ...
              min={0}
              step="0.01"
              disabled={!canManageBudgetSettings}
