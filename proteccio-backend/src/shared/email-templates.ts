...
const _rawFrontendUrl = process.env.FRONTEND_URL || "protecciodata.com";
const FRONTEND_URL = (_rawFrontendUrl.startsWith("http") ? _rawFrontendUrl : `https://${_rawFrontendUrl}`).replace(/\/+$/, "");

const DEFAULT_LOGO_URL = "https://res.cloudinary.com/dlfzzfdx0/image/upload/v1777286182/Brand_title_with_tagline-removebg-preview_jpjpet.png";
 ...
 * Falls back to the default Proteccio logo if no custom logo is set.
 */
...
export function sendNewSubscriberNotification(subscriberEmail: string) {
  ...
  sendEmail({
    subject: "Proteccio - New Subscriber.",
    html: content
  });
}
...
export async function sendJoinTeamInvitation(myName: string, teamName: string, teamId: string, userName: string, toEmail: string, userId: string, projectId?: string) {
  ...
  sendEmail({
    to: [toEmail],
    subject: `${sanitizePlainText(myName)} has invited you to work with ${sanitizePlainText(teamName)} in Proteccio`,
    html: content
  });
}
...
export async function sendResetEmail(toEmail: string, user_id: string, hash: string) {
  ...
  await sendEmail({
    to: [toEmail],
    subject: "Reset your password on Proteccio.",
    html: content
  });
}
