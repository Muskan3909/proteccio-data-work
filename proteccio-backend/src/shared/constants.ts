...
export const ProteccioColorShades = {
...
// Public S3 URL (for browser access to attachments) - can differ from internal S3_URL
// For Docker development: S3_URL=http://minio:9000 (internal), PUBLIC_S3_URL=http://localhost:9000 (browser)
// For production: both should be the same public URL
export const PUBLIC_S3_URL = process.env.PUBLIC_S3_URL || S3_URL;
...
export const PUBLIC_AZURE_STORAGE_URL = process.env.PUBLIC_AZURE_STORAGE_URL || AZURE_STORAGE_URL;
...
export function getPublicStorageUrl() {
  if (STORAGE_PROVIDER === "azure") {
    if (!PUBLIC_AZURE_STORAGE_URL) {
      console.warn("PUBLIC_AZURE_STORAGE_URL is not defined, falling back to AZURE_STORAGE_URL");
      return AZURE_STORAGE_URL || S3_URL;
    }
    return `${PUBLIC_AZURE_STORAGE_URL}/${AZURE_STORAGE_CONTAINER}`;
  }
