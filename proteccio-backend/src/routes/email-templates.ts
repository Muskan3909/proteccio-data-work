...
const TEMPLATES_BASE = "../../Proteccio-email-templates";

router.get("/task-assignee-change", (req: IProteccioRequest, res: IProteccioResponse) => {
  const sampleData = {
...
router.get("/daily-digest", (req: IProteccioRequest, res: IProteccioResponse) => {
...
router.get("/task-moved-to-done", (req: IProteccioRequest, res: IProteccioResponse) => {

  const task = {
    name: "Soloman's Island Project - SIM Registration + Central DB + KYC",
    members: "Dinindu, Dinindu Wanniarachchi, Pasindu Ishan",
    url: "http://localhost:4200/proteccio/projects/6de9c9df-2193-4212-8d64-9dfc36a83ed2?tab=tasks-list&task=f8b3fc45-a28b-4d8f-985e-9e43f8577aa8",
...
router.get("/project-daily-digest", (req: IProteccioRequest, res: IProteccioResponse) => {
  const sampleData: IProjectDigest = {
    ...
    project_url: "/",
    subscribers: [],
    summary: `Here's the "Proteccio" summary | Ceydigital`,
    today_completed: [
      {
        id: "abc123",
        name: "Sample Task",
