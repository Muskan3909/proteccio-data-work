...
import {TASK_PRIORITY_COLOR_ALPHA, ProteccioColorCodes, ProteccioColorShades} from "../shared/constants";

export default class LabelsController extends ProteccioControllerBase {
  @HandleExceptions()
  ...
  @HandleExceptions()
  public static async getByTask(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
    const q = `
  ...
  @HandleExceptions()
  public static async getByProject(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
    const q = `
...
public static async create(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
  ...
  const validColors = [
    ...Object.keys(ProteccioColorShades),
    ...Object.values(ProteccioColorShades).flat(),
  ].map(c => c.toLowerCase());
  ...
  @HandleExceptions()
  public static async updateColor(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
    const q = `UPDATE team_labels
               SET color_code = $3
               WHERE id = $1
                 AND team_id = $2;`;

    const validColors = [...Object.keys(ProteccioColorShades), ...Object.values(ProteccioColorShades).flat()].map(c => c.toLowerCase());
  ...
  @HandleExceptions()
  public static async updateLabel(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
    const updates = [];
