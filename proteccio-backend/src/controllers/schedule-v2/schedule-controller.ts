...
export default class ScheduleControllerV2 extends ProteccioControllerBase {
...
`;
    ...
    public static async updateSettings(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
        try {
            ...
            const updateResult = await db.query(updateWorkingDaysQuery, [req.user?.owner_id]);

            if (updateResult.rowCount === 0) {
                const insertWorkingDaysQuery = `
                ...
                await db.query(insertWorkingDaysQuery, [req.user?.owner_id]);
            }

            // Update working hours
            const updateWorkingHoursQuery = `
...
`;
    ...
    @HandleExceptions()
    public static async getDates(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
    ...
    @HandleExceptions()
    public static async getOrganizationMembers(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
                    ...
                    jsonb_agg(
                        jsonb_build_object(
                            ...
                            'allocated_hours', CASE
                                ...
                                ELSE COALESCE(
                                    (
                                        SELECT SUM(t.total_minutes / 60.0)
                                        FROM tasks t
    ...
    @HandleExceptions()
    public static async getOrganizationMemberProjects(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
        ...
        const getDataq = `
            ...
            member_tasks_in_range AS (
                ...
                -- Tasks that only have a start or end date should still remain visible.
                SELECT DISTINCT
                    t.project_id,
                    COALESCE(t.start_date, $2::DATE) AS start_date,
                    COALESCE(t.end_date, COALESCE($3::DATE, t.start_date, $2::DATE)) AS end_date,
                    t.id AS task_id
                FROM tasks t
                ...
                WHERE pm.team_member_id = $1
                    ...
                    AND COALESCE(t.start_date, $2::DATE) <= COALESCE($3::DATE, t.end_date, t.start_date, $2::DATE)
                    AND COALESCE(t.end_date, COALESCE($3::DATE, t.start_date, $2::DATE)) >= $2::DATE
            ),
                ...
                SELECT
                    ...
                    (
                        ...
                        WHERE pm.team_member_id = $1
                            ...
                            AND t.archived = false
                            AND (t.start_date IS NOT NULL OR t.end_date IS NOT NULL)
                            AND COALESCE(t.start_date, cs.segment_start) <= cs.segment_end
                            AND COALESCE(t.end_date, cs.segment_end) >= cs.segment_start
                    ) AS task_count,
                    ...
                    (
                        SELECT CASE 
                            ...
                            ELSE ROUND(
                                (COUNT(CASE WHEN ts.category_id IN (
                                    SELECT id FROM sys_task_status_categories WHERE is_done = true
                                ) THEN 1 END) * 100.0) / COUNT(DISTINCT t.id)
                            )
                        END
                        FROM tasks t
                        JOIN tasks_assignees ta ON t.id = ta.task_id
            ...
            all_projects_with_segments AS (
                ...
                SELECT
                    ...
                    COALESCE(sws.progress, 0) AS progress,
                    CASE 
    ...
    @HandleExceptions()
    public static async createSchedule(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
    ...
    @HandleExceptions()
    public static async getMemberScheduleSummary(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
        ...
        // Do not filter out tasks that only have a start_date or end_date; they still count
        // toward the selected member's load and log totals.
        const summaryQuery = `
            WITH date_range AS (
                SELECT 
            ...
            allocated_hours AS (
                ...
                LEFT JOIN tasks t ON t.archived = FALSE
                    AND (
                        ...
                        OR (t.start_date IS NULL AND t.end_date >= dr.start_date)
                        OR (t.end_date IS NULL AND t.start_date <= dr.end_date)
                        OR (t.start_date <= dr.end_date AND t.end_date >= dr.start_date)
                    )
                    ${projectFilter}
