...
export default class WorkloadController extends ProteccioControllerBase {
     ...
     */
    @HandleExceptions()
    public static async getMemberWorkload(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
        ...
        const workloadQuery = `
            ...
            allocated_hours AS (
                ...
                GROUP BY tml.team_member_id, tml.user_id, tml.member_name, tml.member_email
            ),
            workload_summary AS (
                SELECT 
                    ah.team_member_id,
     ...
     */
    @HandleExceptions()
    public static async updateResourceAllocation(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
     ...
     */
    @HandleExceptions()
    public static async rebalanceWorkload(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
     ...
     */
    @HandleExceptions()
    public static async getResourceConflicts(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
        // Get organization ID
        const orgQuery = `SELECT id, hours_per_day FROM organizations WHERE user_id = $1 LIMIT 1`;
        const orgResult = await db.query(orgQuery, [req.user?.owner_id]);
     ...
     */
    @HandleExceptions()
    public static async getCapacityReport(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
