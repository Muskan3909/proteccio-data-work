...
export default class ProjectCommentsController extends ProteccioControllerBase {
  ...
  private static async sendMail(config: IMailConfig) {
    ...
    const data: IProjectCommentEmailNotification = {
      ...
      settings_url: `${getBaseUrl()}/proteccio/settings/notifications`,
      project_url: `${getBaseUrl()}/proteccio/projects/${config.projectId}`
    };
  ...
  @HandleExceptions()
  public static async getCountByProjectId(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
