         ...
         (function () {
           try {
             if (window.opener && !window.opener.closed) {
               window.opener.postMessage({ type: "Proteccio:asana-auth", status: "${status}" }, "*");
             }
           } catch (_error) {}
           setTimeout(function () {
...
export default class ImportsController {
  private static getUserId(req: IProteccioRequest): string {
    const id =
  ...
  static create = safeControllerFunction(
    async (req: IProteccioRequest, res: IProteccioResponse) => {
      const {
        provider,
        flowType,
        targetProjectId,
  ...
  static asanaCallback = safeControllerFunction(async (req, res) => {
    ...
    if (existingAccess) {
      ...
      return res.status(200).send(
        buildAsanaCallbackHtml(
          ...
          "This import job already has Asana credentials. You can close this window and return to Proteccio.",
          "already_connected",
        ),
      );
    }
    ...
    if (ref?.auth?.asana?.in_progress) {
      ...
      return res.status(200).send(
        buildAsanaCallbackHtml(
          ...
          "The authorization is currently being processed. Please close this window and return to Proteccio. It will update automatically shortly.",
          "in_progress",
        ),
      );
    }
      ...
      if (status === 400 && data?.error === "invalid_grant") {
        ...
        return res.status(200).send(
          buildAsanaCallbackHtml(
            ...
            'The authorization code appears to be invalid or already used. Please close this window and retry the "Connect" flow from Proteccio (create a fresh import job and click Connect).',
            "failed",
          ),
        );
      }
    ...
    return res.status(200).send(
      buildAsanaCallbackHtml(
        "Asana connected",
        "You can close this window and return to Proteccio.",
        "success",
      ),
    );
  });
