...
export default class SupportController extends ProteccioControllerBase {

  @HandleExceptions()
  public static async contactSupport(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
    try {
      const {user} = req;
      if (!user) {
