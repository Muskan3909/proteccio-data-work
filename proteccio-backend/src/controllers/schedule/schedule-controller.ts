...
export default class ScheduleControllerV2 extends ScheduleTasksControllerBase {
  ...
  // Migrate data
  @HandleExceptions()
  public static async migrate(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
...
AND p.id NOT IN (SELECT project_id FROM archived_projects)`;
  ...
  @HandleExceptions()
  public static async createDateRange(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
  ...
  @HandleExceptions()
  public static async getSingleProjectIndicator(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
  ...
  @HandleExceptions()
  public static async getSingleProjectMember(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
  ...
  @HandleExceptions()
  public static async deleteMemberAllocations(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
  ...
  private static getQuery(userId: string, projectId: string, options: ParsedQs): { query: string; params: any[] } {
    ...
    if (options.startDate && options.endDate) {
      ...
      // 1) overlap the selected range, or
      // 2) are unscheduled / missing dates and should still remain visible in the schedule groups.
      dateRangeFilter = `(
        (start_date IS NULL OR end_date IS NULL)
        OR (
          start_date <= $${paramOffset}
          AND (end_date IS NULL OR end_date >= $${paramOffset + 1})
        )
      )`;
  ...
  @HandleExceptions()
  public static async getList(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
  ...
  @HandleExceptions()
  public static async getTasksOnly(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
    const { query: q, params } = ScheduleControllerV2.getQuery(req.user?.id as string, req.params.id, req.query);
    const result = await db.query(q, params);
