...
export default class TeamLeadReportsController {
  ...
  public static async getMyTeamMembers(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
    try {
  ...
  public static async getTeamTimeLogsSummary(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
    try {
      const userId = req.user?.id;
      const teamId = req.user?.team_id;
  ...
  public static async getMemberDetailedTimeLogs(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
    try {
      const userId = req.user?.id;
      const teamId = req.user?.team_id;
  ...
  public static async getTeamPerformanceStats(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
    try {
      const userId = req.user?.id;
      const teamId = req.user?.team_id;
      const { startDate, endDate } = req.query;
