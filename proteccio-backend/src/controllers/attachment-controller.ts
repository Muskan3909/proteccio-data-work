...
import { getPublicStorageUrl } from "../shared/constants";
...
export default class AttachmentController extends ProteccioControllerBase {

  @HandleExceptions()
  public static async createTaskAttachment(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
    ...
    const result = await db.query(q, [
      ...
      size,
      type,
      `${getPublicStorageUrl()}/${getRootDir()}`
    ]);
  ...
  @HandleExceptions()
  public static async createAvatarAttachment(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
  ...
  @HandleExceptions()
  public static async deleteAvatarAttachment(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
  ...
  public static async get(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
    const q = `
      ...
      WHERE task_id = $1;
    `;
    const result = await db.query(q, [req.params.id, `${getPublicStorageUrl()}/${getRootDir()}`]);
  ...
  @HandleExceptions()
  public static async getByProjectId(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
    ...
    `;
    const result = await db.query(q, [req.params.id, `${getPublicStorageUrl()}/${getRootDir()}`, size, offset]);
    const [data] = result.rows;
