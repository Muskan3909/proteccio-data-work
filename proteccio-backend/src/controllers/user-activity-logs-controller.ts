...
export default class UserActivityLogsController extends ProteccioControllerBase {
    @HandleExceptions()
    public static async getRecentTasks(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
        if (!req.user) {
    ...
    @HandleExceptions()
    public static async getTimeLoggedTasks(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
        if (!req.user) {
            return res.status(401).send(new ServerResponse(false, null, "Unauthorized"));
        }
