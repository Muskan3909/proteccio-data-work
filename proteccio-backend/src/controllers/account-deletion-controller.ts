...
export default class AccountDeletionController extends ProteccioControllerBase {

  @HandleExceptions()
  public static async requestDeletion(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
    try {
      const { user } = req;
      if (!user) {
  ...
  @HandleExceptions()
  public static async cancelDeletion(req: IProteccioRequest, res: IProteccioResponse): Promise<IProteccioResponse> {
    try {
      const { user } = req;
      if (!user) {
        return res.status(401).send(new ServerResponse(false, "Unauthorized"));
