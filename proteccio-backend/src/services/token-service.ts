...
class TokenService {
  ...
  generateClientToken(payload: ClientTokenPayload): string {
    return jwt.sign(payload, this.SECRET_KEY, {
      expiresIn: "24h",
      issuer: "proteccio-client-portal",
      audience: "client"
    });
  }
  ...
  generateInviteToken(payload?: InviteTokenPayload): string {
    // Generate a secure Base62 token with "wli" prefix (Proteccio Invite)
  ...
  async acceptInvitation(token: string, userData: {
    password: string;
    name: string;
    userId?: string | null; // Optional Proteccio user ID for linking
  }): Promise<any> {
    const invitation = await this.getInvitationByToken(token);
    if (!invitation) {
    ...
    try {
      ...
      if (emailExistsCheck.rows.length > 0) {
        ...
        if (userData.userId) {
          // Linking existing Proteccio user
          await client.query(
            `UPDATE client_users 
        ...
        if (userData.userId) {
          // Linking existing Proteccio user - no password_hash needed for client_users table
          createUserQuery = `
  ...
  async authenticateClient(email: string, password: string): Promise<any> {
    try {
      ...
      // Check if this is a linked Proteccio user (has user_id)
      if (clientUser.user_id) {
        // Authenticate against Proteccio users table
        const ProteccioAuthQuery = `
          SELECT u.id, u.email, u.name, u.password
          FROM users u
          WHERE u.id = $1 AND u.is_deleted = FALSE
        `;
        const ProteccioUserResult = await db.query(ProteccioAuthQuery, [clientUser.user_id]);

        if (ProteccioUserResult.rows.length === 0) {
          return null; // Linked Proteccio user not found
        }

        const ProteccioUser = ProteccioUserResult.rows[0];

        if (!ProteccioUser.password) {
          return null; // No password set for linked user
        }

        // Verify password against Proteccio user password (bcrypt)
        const passwordMatch = bcrypt.compareSync(password, ProteccioUser.password);
        if (passwordMatch) {
