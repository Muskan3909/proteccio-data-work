...
const allowedOrigins = [
  isProduction()
    ? [
        `http://localhost:5000`,
        `http://127.0.0.1:5000`,
        `https://app.protecciodata.com`,
        `https://www.app.protecciodata.com`,
        `https://client.protecciodata.com`,
        `https://clients.protecciodata.com`,
  ...
  try {
    ...
    app.use(
      ...
      swaggerUi.setup(swaggerDocument, {
        ...
        customSiteTitle: "Proteccio Data API Documentation",
        swaggerOptions: {
...
app.get("*", (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!fs.existsSync(frontendEntry)) return next();
  res.sendFile(
    frontendEntry,
  );
});
