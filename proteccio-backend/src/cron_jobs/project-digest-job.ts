...
function updateTaskUrls(projectId: string, tasks: IProjectDigestTask[]) {
  ...
  for (const task of tasks) {
    task.url = `${baseUrl}/proteccio/projects/${projectId}?tab=tasks-list&task=${task.id}`;
  }
}
...
function updateMetadata(project: IProjectDigest, subscriberName: string) {
  ...
  project.settings_url = `${getBaseUrl()}/proteccio/settings/notifications`;
  project.project_url = `${getBaseUrl()}/proteccio/projects/${project.id}?tab=tasks-list`;
}
...
async function onProjectDigestJobTick() {
  ...
  isRunning = true;
  try {
    const lockResult = await db.query("SELECT pg_try_advisory_lock(hashtext($1)) AS locked;", ["Proteccio-project-digest"]);
    ...
    if (hasLock) {
      try {
        await db.query("SELECT pg_advisory_unlock(hashtext($1));", ["Proteccio-project-digest"]);
      } catch (error) {
        log_error(error);
      }
    }
