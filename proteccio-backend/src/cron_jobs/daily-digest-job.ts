...
async function onDailyDigestJobTick() {
  ...
  isRunning = true;
  try {
    const lockResult = await db.query("SELECT pg_try_advisory_lock(hashtext($1)) AS locked;", ["Proteccio-daily-digest"]);
    ...
    for (const digest of dataset) {
      ...
      digest.base_url = `${getBaseUrl()}/proteccio`;
      digest.settings_url = `${getBaseUrl()}/proteccio/settings/notifications`;
    ...
    if (hasLock) {
      try {
        await db.query("SELECT pg_advisory_unlock(hashtext($1));", ["Proteccio-daily-digest"]);
      } catch (error) {
        log_error(error);
      }
    }
