...
function getModel(model: ITaskAssignmentsModel): ITaskAssignmentsModel {
  ...
  mappedModel.url = `${getBaseUrl()}/proteccio/team/member/${mappedModel.team_member_id}`;
  mappedModel.settings_url = `${getBaseUrl()}/proteccio/settings/notifications`;
...
async function onNotificationJobTick() {
  ...
  isRunning = true;
  try {
    const lockResult = await db.query("SELECT pg_try_advisory_lock(hashtext($1)) AS locked;", ["Proteccio-email-notifications"]);
    ...
    if (hasLock) {
      try {
        await db.query("SELECT pg_advisory_unlock(hashtext($1));", ["Proteccio-email-notifications"]);
      } catch (error) {
        log_error(error);
      }
    }
