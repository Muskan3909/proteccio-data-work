import ProteccioControllerBase from "../controllers/Proteccio-controller-base";
...
/** HandleExceptions can only be used with an instance of ProteccioControllerBase. */
export default function HandleExceptions(options?: IExceptionHandlerConfig) {
  const opt = mergeWithDefaults(options);
  return (target: any, key: string, descriptor: PropertyDescriptor) => {
    if (!(target.prototype instanceof ProteccioControllerBase))
      throw new Error("@HandleExceptions can only be used with an instance of ProteccioControllerBase.");
