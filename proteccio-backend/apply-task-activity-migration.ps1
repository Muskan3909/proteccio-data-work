...
if (-not (Test-Path "database/migrations")) {
    Write-Host "ERROR: Please run this script from the Proteccio-backend directory" -ForegroundColor Red
    exit 1
}
...
$dbName = Read-Host "Database Name (default: Proteccio)"
if ([string]::IsNullOrWhiteSpace($dbName)) { $dbName = "Proteccio" }
