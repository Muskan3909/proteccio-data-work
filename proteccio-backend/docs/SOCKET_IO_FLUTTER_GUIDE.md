# Proteccio Socket.IO Integration Guide for Flutter/Dart
...
Proteccio uses Socket.IO for real-time bidirectional communication between clients and the server. This guide provides comprehensive documentation for implementing Socket.IO events in Flutter/Dart applications.
...
This guide covers all Socket.IO events in the Proteccio backend. Key points:
...
For additional support, refer to the backend source code in `Proteccio-backend/src/socket.io/`.
