-- Migration: Add user_id to client_users for Proteccio User Linking
-- Description: Allows Proteccio users to access client portal with their existing credentials
-- Date: 2025-10-02
-- Version: 2.3.0
...
-- Add unique constraint to prevent same Proteccio user from being linked to same client multiple times
...
-- Make password_hash nullable since Proteccio users authenticate via users table
...
COMMENT ON COLUMN client_users.user_id IS 'Links to Proteccio user for single sign-on. NULL for standalone client portal accounts.';
COMMENT ON CONSTRAINT client_users_auth_check ON client_users IS 'Ensures authentication method exists: either linked Proteccio user (user_id) or client portal password (password_hash)';
