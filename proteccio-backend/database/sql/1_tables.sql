...
CREATE TABLE IF NOT EXISTS organizations (
    ...
    business_plan_override       BOOLEAN                  DEFAULT FALSE,
...
ALTER TABLE sys_license_types
    ADD CONSTRAINT sys_license_types_pk
        PRIMARY KEY (id);

ALTER TABLE sys_license_types
    ADD CONSTRAINT sys_license_types_key_uindex
        UNIQUE (key);
...
-- ============================================================
-- Licensing/subscription stub tables (Community Edition)
-- These support the deserialize_user() / register_user() functions
-- and related billing-adjacent queries. CE installs have no real
-- billing provider, so these stay empty; queries against them
-- simply resolve to NULL/no-match, which the functions handle.
-- ============================================================
...
ALTER TABLE licensing_user_subscriptions
    ADD CONSTRAINT licensing_user_subscriptions_pk
        PRIMARY KEY (id);
...
ALTER TABLE licensing_custom_subs
    ADD CONSTRAINT licensing_custom_subs_pk
        PRIMARY KEY (id);
...
ALTER TABLE licensing_plan_trials
    ADD CONSTRAINT licensing_plan_trials_pk
        PRIMARY KEY (id);
...
CREATE TABLE IF NOT EXISTS Proteccio_alerts (
    description TEXT NOT NULL,
    type        TEXT NOT NULL,
    active      BOOLEAN DEFAULT FALSE
);

ALTER TABLE Proteccio_alerts
    ADD CONSTRAINT Proteccio_alerts_type_check
...
ON tasks (schedule_id, immutable_date_utc(end_date))
