...
CREATE OR REPLACE FUNCTION normalize_project_progress_flags(_manual BOOLEAN, _weighted BOOLEAN, _time BOOLEAN)
    RETURNS TABLE (
...
$$
DECLARE
    _manual_value BOOLEAN := COALESCE(_manual, FALSE);
    _weighted_value BOOLEAN := COALESCE(_weighted, FALSE);
    _time_value BOOLEAN := COALESCE(_time, FALSE);
    _active_count INTEGER;
BEGIN
    _active_count := (
        CASE WHEN _manual_value THEN 1 ELSE 0 END +
        CASE WHEN _weighted_value THEN 1 ELSE 0 END +
        CASE WHEN _time_value THEN 1 ELSE 0 END
    );

    IF _active_count <= 1 THEN
        use_manual_progress := _manual_value;
        use_weighted_progress := _weighted_value;
        use_time_progress := _time_value;
        RETURN NEXT;
        RETURN;
    END IF;

    IF _manual_value THEN
...
BEGIN
    ...
    FROM normalize_project_progress_flags(
    ...
    VALUES (_project_name, (_body ->> 'key')::TEXT, (_body ->> 'notes')::TEXT, (_body ->> 'color_code')::TEXT, _team_id,
            ...
            _progress.use_manual_progress,
            _progress.use_weighted_progress,
            _progress.use_time_progress,
...
BEGIN
    ...
    alerts_data AS (
        ...
        FROM (SELECT description, type FROM Proteccio_alerts WHERE active IS TRUE) alert_rec
    ),
...
BEGIN
    ...
    SET name                     = _project_name,
        ...
        use_manual_progress      = _progress.use_manual_progress,
        use_weighted_progress    = _progress.use_weighted_progress,
        use_time_progress        = _progress.use_time_progress,
