...
BEGIN
    ...
    task_allocations AS (
        SELECT 
            ds.date,
            COALESCE(SUM(t.total_minutes) / 60.0, 0) AS allocated_hours,
            COALESCE(
                jsonb_agg(
                    jsonb_build_object(
                        'project_id', t.project_id,
                        'project_name', p.name,
                        'allocated_hours', ROUND((t.total_minutes / 60.0)::NUMERIC, 2),
                        'color_code', p.color_code
                    )
        ...
        FROM date_series ds
        JOIN tasks_assignees ta ON ta.team_member_id = p_team_member_id
        JOIN tasks t ON t.id = ta.task_id
        JOIN projects p ON p.id = t.project_id
        WHERE t.archived = false
          AND (
              (t.start_date IS NULL AND t.end_date IS NULL)
              OR (t.start_date IS NULL AND t.end_date >= ds.date)
              OR (t.end_date IS NULL AND t.start_date <= ds.date)
              OR (t.start_date <= ds.date AND t.end_date >= ds.date)
          )
        GROUP BY ds.date
    ),
    daily_allocations AS (
        SELECT
            ds.date,
            COALESCE(pa.allocated_hours, ta.allocated_hours, 0) AS allocated_hours,
            COALESCE(pa.projects, ta.projects, '[]'::jsonb) AS projects
        FROM date_series ds
        LEFT JOIN project_allocations pa ON pa.date = ds.date
        LEFT JOIN task_allocations ta ON ta.date = ds.date
    ),
    time_off_check AS (
        SELECT 
            ds.date,
