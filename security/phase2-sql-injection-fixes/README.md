...
- Location: `/Proteccio-backend/src/shared/sql-helpers.ts`
- Provides secure query building methods
- Replaces unsafe `flatString()` pattern

✅ **Critical Fix: Socket.IO Timer Command**
- File: `/Proteccio-backend/src/socket.io/commands/on-task-timer-stop.ts`
...
```bash
# Test with sqlmap
sqlmap -u "https://api.Proteccio.com/api/v1/tasks?statuses=test" \
  --cookie="Proteccio.sid=..." \
  --level=5 \
  --risk=3
...
- **SQL Helpers:** `/Proteccio-backend/src/shared/sql-helpers.ts`
