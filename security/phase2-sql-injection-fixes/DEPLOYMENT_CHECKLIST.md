...
```bash
# Backup database
pg_dump Proteccio_db > backup_pre_phase2_$(date +%Y%m%d).sql
...
pg_dump Proteccio_prod > backup_prod_phase2_$(date +%Y%m%d).sql
...
# 3. Deploy
cd /path/to/Proteccio-backend-prod
...
psql Proteccio_prod < backup_prod_phase2_YYYYMMDD.sql
```
...
**If Issues Arise:**
1. Check logs: `pm2 logs Proteccio-backend`
2. Check database: `psql Proteccio_db`
