...
export const getApiUrl = (): string => {
  ...
  // Example: ncinga.protecciodata.com -> api.ncinga.protecciodata.com
  // Example: app.protecciodata.com -> api.protecciodata.com
...
export const getClientPortalBaseUrl = (): string => {
  ...
  // For production, derive from current hostname.
  // If on app.protecciodata.com, client portal runs on client.protecciodata.com.
  if (hostname.startsWith('app.')) {
    const clientHostname = `client.${hostname.slice(4)}`;
    return `${protocol}//${clientHostname}`;
  }

  // For other environments, use the same hostname
