...
export const normalizeCurrency = (currencyCode?: string | null): string => {
  ...
  const normalized = currencyCode.toString().trim().toLowerCase();
...
export const getCurrencySymbol = (currencyCode: string): string => {
  ...
  const currency = CURRENCY_OPTIONS.find(c => c.value === normalizedCode);
  return currency?.symbol || normalizedCode.toUpperCase();
};

export const getCurrencyLabel = (currencyCode: string): string => {
  const normalizedCode = normalizeCurrency(currencyCode);
  const currency = CURRENCY_OPTIONS.find(c => c.value === normalizedCode);
  return currency?.label || normalizedCode.toUpperCase();
};
