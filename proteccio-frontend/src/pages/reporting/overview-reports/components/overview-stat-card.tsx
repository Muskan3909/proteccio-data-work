...
const OverviewStatCard = React.memo(
  ({ icon, title, children, loading = false }: InsightCardProps) => {
    ...
    const isDarkMode =
      token.colorBgContainer === '#131B27' ||
      token.colorBgBase === '#0B1221' ||
      token.colorBgElevated === '#131B27' ||
    ...
    const cardStyles = useMemo(
      () => ({
        body: {
          padding: '24px',
          background: isDarkMode ? '#131B27 !important' : '#ffffff !important',
        },
      }),
      [isDarkMode]
    );
    ...
    const cardContainerStyle = useMemo(
      () => ({
        ...
        cursor: 'default',
        backgroundColor: isDarkMode ? '#131B27 !important' : '#ffffff !important',
      }),
      [isDarkMode]
    );
    ...
    const iconContainerStyle = useMemo(
      () => ({
        padding: '0',
        borderRadius: '12px',
        background: isDarkMode ? '#173b3b' : '#e8f7f2',
        display: 'flex',
        alignItems: 'center',
    ...
    return (
      <div
        ...
        style={{
          backgroundColor: isDarkMode ? '#131B27' : '#ffffff',
          border: isDarkMode ? '1px solid #303030' : '1px solid #f0f0f0',
          borderRadius: '0px',
          boxShadow: isDarkMode ? '0 2px 8px rgba(0, 0, 0, 0.3)' : '0 2px 8px rgba(0, 0, 0, 0.06)',
          transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
      ...
      >
        ...
        >
          <Flex gap={16} align="flex-start">
            <div className="overview-stat-icon" style={iconContainerStyle}>{icon}</div>
