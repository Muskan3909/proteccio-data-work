...
const ReportingSider: React.FC<ReportingSiderProps> = ({ collapsed = false, onToggleCollapse }) => {
        ...
        if (item.children) {
          items.push({
            ...
            children: item.children.map(child => ({
              ...
              icon: child.icon,
              label: (
                <Link to={`/proteccio/reporting/${child.endpoint}`}>
          ...
          items.push({
            ...
            icon: item.icon,
            label: (
              <Link to={`/proteccio/reporting/${item.endpoint}`}>
  ...
  const activeKey = useMemo(() => {
    const afterProteccioString = location.pathname?.split('/proteccio/reporting/')[1];
