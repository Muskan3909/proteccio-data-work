...
import { Proteccio_REDIRECT_PROJ_KEY } from '@/shared/constants';
...
const SignupPage = () => {
  ...
  const setProjectId = (projectId: string) => {
    if (!projectId) {
      localStorage.removeItem(Proteccio_REDIRECT_PROJ_KEY);
      return;
    }
    localStorage.setItem(Proteccio_REDIRECT_PROJ_KEY, projectId);
  };

  const getProjectId = () => {
    return localStorage.getItem(Proteccio_REDIRECT_PROJ_KEY);
  };
      ...
      >
        ...
        <Form.Item>
          <Typography.Paragraph style={{ fontSize: 14 }}>
            {t('bySigningUpText')}{' '}
            <a href="https://protecciodata.com/privacy" target="_blank" rel="noopener noreferrer">
              {t('privacyPolicyLink')}
            </a>{' '}
            {t('andText')}{' '}
            <a href="https://protecciodata.com/terms" target="_blank" rel="noopener noreferrer">
              {t('termsOfUseLink')}
            </a>
            .
        ...
        <Form.Item>
          <Space>
            ...
            <Link
              to="/auth/login"
              className="ant-typography ant-typography-link auth-login-link"
              style={{ fontSize: 14 }}
            >
              {t('loginButton')}
            </Link>
