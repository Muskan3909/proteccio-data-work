...
const ProjectList: React.FC = () => {
  ...
  const navigateToProject = useCallback(
    (project_id: string | undefined, default_view: string | undefined) => {
      if (project_id) {
        navigate(
          `/proteccio/projects/${project_id}?tab=${default_view === 'BOARD' ? 'board' : 'tasks-list'}&pinned_tab=${default_view === 'BOARD' ? 'board' : 'tasks-list'}`
        );
      }
    },
    [navigate]
  ...
  return (
    <div className="proteccio-projects-page" style={{ minHeight: '90vh' }}>
      <ProteccioPageHeader
