...
import { CURRENCY_OPTIONS, DEFAULT_CURRENCY, normalizeCurrency } from '@/shared/currencies';
...
const ProjectViewFinance = () => {
  ...
  const projectCurrency = normalizeCurrency(
    financeProject?.currency || project?.currency || DEFAULT_CURRENCY
  );
  ...
  const refreshFinanceData = useCallback(
    (resetExpansions = false) => {
      if (projectId && hasBusinessAccess) {
        dispatch(
          fetchProjectFinancesSilent({
            projectId,
            groupBy: activeGroup,
    ...
    [projectId, activeGroup, billableFilter, dispatch, hasBusinessAccess]
  );
  ...
  }, [projectId, activeGroup, billableFilter, dispatch, refreshTimestamp, hasBusinessAccess]);
