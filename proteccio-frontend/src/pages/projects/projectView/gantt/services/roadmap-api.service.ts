...
export const transformToGanttTasks = (
  ...
  apiPhases.forEach(phase => {
    ...
    const phaseMilestone: GanttTask = {
      ...
      start_date: phaseStartDate,
      end_date: phaseEndDate,
      progress: phase.done_progress || 0,
      level: 0,
      expanded: true,
  ...
  const unmappedPhaseTasks = unassignedTasks.map(task => transformTask(task, 1));
  const unmappedTotal = unmappedPhaseTasks.length;
  const unmappedDone = unmappedPhaseTasks.filter(task => task.progress === 100).length;
  const unmappedProgress = unmappedTotal > 0 ? Math.round((unmappedDone / unmappedTotal) * 100) : 0;
