...
const TaskRow: React.FC<TaskRowProps & { dragAttributes?: any; dragListeners?: any }> = memo(
  ({
    ...
    const phaseCompletion = useMemo(() => {
      ...
      // Prefer backend-provided done_progress if available (already a percentage 0-100)
      if (typeof (task as any).done_progress === 'number') {
        return Math.round((task as any).done_progress);
      }
      
      // Fallback: calculate from children if available
