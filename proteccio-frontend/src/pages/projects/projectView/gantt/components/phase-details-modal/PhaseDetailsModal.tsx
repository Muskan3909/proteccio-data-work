...
const PhaseDetailsModal: React.FC<PhaseDetailsModalProps> = ({
  ...
  const phaseStats = useMemo(() => {
    ...
    if (hasBackendAggregates) {
      ...
      const doneProgress = (localPhase as any).done_progress as number;
      
      // done_progress is already a percentage (0-100), not a count
      let completionPercentage = Math.round(doneProgress);
      
      // Clamp to 0-100 range to handle any data inconsistencies
      completionPercentage = Math.max(0, Math.min(100, completionPercentage));
      
      const completedTasks = totalTasks > 0 
        ? Math.round((completionPercentage / 100) * totalTasks) 
        : 0;
