...
export function isChunkLoadFailureError(error: unknown): boolean {
  ...
  if (lowerValue.includes('failed to fetch dynamically imported module')) return true;
  if (lowerValue.includes('loading chunk')) return true;
  if (lowerValue.includes('loading css chunk')) return true;
  if (lowerValue.includes('chunkloaderror')) return true;

  return false;
}

export function shouldTriggerChunkReload(message: string | undefined): boolean {
  if (!message) return false;

  const normalized = message.toLowerCase();
  if (normalized.includes('unable to preload css')) {
    return false;
  }
...
export class ChunkErrorHandler {
  ...
  static isChunkError(error: any): boolean {
    return isChunkLoadFailureError(error);
  }
