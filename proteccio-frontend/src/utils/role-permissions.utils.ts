...
export const getRoleDefinition = (roleName?: string, isOwner?: boolean): IRoleDefinition => {
  const normalizedRole = normalizeRoleName(roleName, isOwner);
  return ROLE_DEFINITIONS[normalizedRole] ?? ROLE_DEFINITIONS[ROLE_NAMES.MEMBER];
};
...
export function canManageUserRole(
  ...
  return getRoleDefinition(actorRole).manageableRoles.includes(targetRole);
}
...
export function getAvailableRoleOptions(
  ...
  const actorRole = normalizeRoleName(currentUserRole, isOwner);
  return getRoleDefinition(actorRole).assignableRoles.map(roleName => {
