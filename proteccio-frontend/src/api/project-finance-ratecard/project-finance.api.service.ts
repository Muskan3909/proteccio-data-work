...
const projectsRootUrl = `${API_BASE_URL}/projects`;
...
export const projectFinanceApiService = {
  getProjectTasks: async (
    ...
    billableFilter: BillableFilterType = 'all'
    ...
    try {
      ...
      if (error?.response?.status === 404) {
        return {
          ...
          body: { groups: [], project_rate_cards: [], project: null },
          message: '',
        } as IServerResponse<IProjectFinanceResponse>;
      }
      throw error;
    }
  },
  ...
  updateProjectCurrency: async (
    ...
    const response = await apiClient.put<IServerResponse<any>>(
      `${projectsRootUrl}/${projectId}/currency`,
      { currency }
    );
    return response.data;
  },
  ...
  updateProjectBudget: async (projectId: string, budget: number): Promise<IServerResponse<any>> => {
    const response = await apiClient.put<IServerResponse<any>>(
      `${projectsRootUrl}/${projectId}/budget`,
      { budget }
    );
    return response.data;
  },
