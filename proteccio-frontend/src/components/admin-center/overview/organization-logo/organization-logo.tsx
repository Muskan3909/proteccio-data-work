...
const OrganizationLogo: React.FC<OrganizationLogoProps> = ({
  ...
  const hasLogoFeatureAccess = true;
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (res.done) {
        ...
        refetch();
        message.success(t('logoUploadSuccess', { defaultValue: 'Logo updated successfully' }));
      } else {
        const errorMessage = res.message || t('logoUploadError');
  ...
  const logoPreview = (
    <div
      ...
      style={{
        ...
        backgroundColor: themeMode === 'dark' ? '#131B27' : '#fafafa',
      ...
      onMouseLeave={e => {
        if (!previewUrl && hasLogoFeatureAccess) {
          ...
          e.currentTarget.style.backgroundColor = themeMode === 'dark' ? '#131B27' : '#fafafa';
        }
      }}
    >
      {uploading && (
