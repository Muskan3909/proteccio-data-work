...
const FinanceTableWrapper: React.FC<FinanceTableWrapperProps> = ({ activeTablesList, loading }) => {
  ...
  const currency = useAppSelector(
    state => state.projectFinancesReducer.project?.currency || DEFAULT_CURRENCY
  ).toUpperCase();
            ...
            {hasAnyTasks && (
              <tr
                style={{
                  height: 56,
                  fontWeight: 500,
                  backgroundColor: themeWiseColor('#fbfbfb', '#0B1221', themeMode),
                }}
              >
