...
const RateCardTable: React.FC = () => {
  ...
  const handleSelectJobTitle = async (jobTitleId: string, selectedJobTitle?: IJobTitle) => {
    const jobTitle = selectedJobTitle || jobTitles.find(jt => jt.id === jobTitleId);
  ...
  const handleCreateJobTitle = async () => {
    ...
    try {
      const response = await jobTitlesApiService.createJobTitle({ name });
      if (response.done && response.body?.id) {
        await loadJobTitles();
        await handleSelectJobTitle(response.body.id, response.body);
        setNewJobTitleName('');
      }
    } catch (error) {
        ...
        if (addingRow && index === roles.length) {
          return (
            <Select
              ...
              notFoundContent="No job titles configured"
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, position: 'relative' }}>
          <Avatar.Group>
            {memberscol?.map((memberId, i) => {
              const member = members.find(m => (m.team_member_id || m.id) === memberId);
              return member ? (
