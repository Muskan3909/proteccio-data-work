...
import { evt_projects_create } from '@/shared/proteccio-analytics-events';
...
import { CURRENCY_OPTIONS, normalizeCurrency } from '@/shared/currencies';
import { projectFinanceApiService } from '@/api/project-finance-ratecard/project-finance.api.service';
import { isCurrentUserTeamLead } from '@/utils/team-lead-utils';
import { normalizeProjectProgressMode } from '@/utils/project-progress-mode';
...
export const ProjectDrawer = ({ onClose }: { onClose: () => void }) => {
  ...
  const defaultFormValues = useMemo(() => {
    const normalizedProgress = normalizeProjectProgressMode({
    ...
    return {
      ...
      currency: normalizeCurrency(project?.currency || 'USD'),
      ...
      use_manual_progress: normalizedProgress.use_manual_progress,
      use_weighted_progress: normalizedProgress.use_weighted_progress,
      use_time_progress: normalizedProgress.use_time_progress,
  ...
  const canManageBudgetSettings =
    isProjectManager || isOwnerorAdmin || isCurrentUserTeamLead(currentSession);
  ...
  const handleUpgradeClick = useCallback(() => {
    window.location.href = '/proteccio/admin-center/billing';
  }, []);
  ...
  const handleFormSubmit = async (values: any) => {
    try {
      ...
      if (response?.data?.done) {
        if (editMode && projectId && canManageBudgetSettings) {
          const selectedBudget = Number(values.budget ?? 0);
          const selectedCurrency = normalizeCurrency(values.currency || 'USD').toUpperCase();
          const currentBudget = Number(project?.budget ?? 0);
          const currentCurrency = normalizeCurrency(project?.currency || 'USD').toUpperCase();
        ...
        if (!editMode) {
          ...
          navigate(
            `/proteccio/projects/${response.data.body.id}?tab=tasks-list&pinned_tab=tasks-list`
          );
          setTimeout(() => {
