...
const ProjectBudgetSettingsDrawer: React.FC<ProjectBudgetSettingsDrawerProps> = ({
  ...
  const initialValues = {
    ...
    currency: normalizeCurrency(financeProject?.currency || project?.currency || 'USD'),
  };
  ...
  const handleSave = async () => {
    try {
      ...
      if (normalizedCurrency !== normalizeCurrency(financeProject?.currency || project?.currency)) {
        await projectFinanceApiService.updateProjectCurrency(
          projectId,
          normalizedCurrency.toUpperCase()
        );
        dispatch(updateProjectCurrency(normalizedCurrency));
        dispatch(updateProjectFinanceCurrency(normalizedCurrency));
      }
