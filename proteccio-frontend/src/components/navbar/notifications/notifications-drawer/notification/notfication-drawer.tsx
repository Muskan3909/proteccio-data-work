...
import { IProteccioNotification } from '@/types/notifications/notifications.types';
...
const NotificationDrawer = () => {
  ...
  const createPush = (message: string, title: string, teamId: string | null, url?: string) => {
    if (Notification.permission === 'granted' && showBrowserPush) {
      const img = '/assets/icons/icon-192x192.png';
  ...
  const handleNotificationsUpdate = async (notification: IProteccioNotification) => {
    dispatch(fetchNotifications(notificationType));
    dispatch(fetchInvitations());
    dispatch(fetchUnreadCount()); // Fetch updated unread count

    if (isPushEnabled()) {
      const title = notification.team ? `${notification.team} | Proteccio` : 'Proteccio';
  ...
  const handleTeamInvitationsUpdate = async (data: ITeamInvitationViewModel) => {
    const notification: IProteccioNotification = {
      id: data.id || '',
      team: data.team_name || '',
  ...
  const handleTeamMemberRemoved = async (data: { teamId: string; message: string }) => {
    ...
    if (isPushEnabled()) {
      createPush(notification.message, 'Proteccio', notification.team_id || null);
    }
  ...
  const goToUrl = async (event: React.MouseEvent, notification: IProteccioNotification) => {
  ...
  return (
    ...
    >
      ...
      {notifications && notifications.length > 0 ? (
        ...
        <Empty
          image={<span className="proteccio-notification-empty-icon" aria-hidden="true">🔔</span>}
