...
const GranttChart = React.forwardRef(({ type, date }: { type: string; date: Date }, ref) => {
  ...
  // Fetch workload summary data for total allocated hours
  const {
  ...
  const isRefetching = teamFetching || dateFetching || capacityFetching || workloadFetching;
  const dayCount =
  ...
  // Helper function to get total allocated hours for a member from workload summary
  const getTotalAllocatedHours = (memberId: string) => {
    const normalizedMemberId = String(memberId);
    const memberWorkload = workloadData?.find((m: any) =>
      String(m.id) === normalizedMemberId ||
      String(m.userId) === normalizedMemberId ||
      String(m.team_member_id) === normalizedMemberId
    );

    return typeof memberWorkload?.allocatedHours === 'number' ? memberWorkload.allocatedHours : 0;
  };
  ...
  useEffect(() => {
    ...
    setMemberProjects({});
    setExpandedMemberId(null);
  }, [date, type, refetchTeam, refetchDates, refetchCapacity, refetchWorkload, formattedDate]);
    ...
    return (
      <div
        style={{
          ...
          borderRadius: '18px',
          backgroundColor: themeMode === 'dark' ? '#081521' : '',
          padding: '16px',
        }}
      >
        <div>
  ...
  return (
    <>
      ...
      <div
        className="proteccio-schedule-grid"
        style={{
          display: 'grid',
          gridTemplateColumns: '276px 1fr',
          overflow: 'hidden',
      ...
      >
        ...
        <div
          style={{
            background: themeWiseColor('#fff', '#081521', themeMode),
          }}
        ...
        <div style={{ overflow: 'auto', position: 'relative', flex: 1, minWidth: 0 }}>
          <div
            ...
            style={{
              ...
              right: 0,
              zIndex: 100,
              backgroundColor: themeWiseColor('#fff', '#081521', themeMode),
              scrollbarWidth: 'none',
                        ...
                        dateObj.days.map((day: any, dayIndex: number) => {
                          ...
                          return (
                            <div
                              ...
                              style={{
                                background: day.isWeekend ? 'rgba(255, 255, 255, 0.025)' : '',
                                color: day.isToday ? '#fff' : '',
                                height: 84,
                              }}
                            >
                              <DayAllocationCell
                                ...
                                isWeekend={day.isWeekend}                                totalAllocatedHours={getTotalAllocatedHours(memberId)}                              />
                            </div>
                          );
                        })
                      )}
