...
const DayAllocationCell = ({
  ...
  const handleClick = () => {
    if (isInteractive && memberId) {
      ...
      dispatch(
        setSelectedDateRange({
          start: selectedDay.startOf('month').format('YYYY-MM-DD'),
          end: selectedDay.endOf('month').format('YYYY-MM-DD'),
        })
      );
      dispatch(setSelectedProject(null));
      dispatch(setSelectedSegmentData(null));
  ...
  const safeRangeTotal = Number.isFinite(totalAllocatedHours) ? totalAllocatedHours : 0;
  const safeDailyAllocated = Number.isFinite(effectiveData.allocated_hours)
    ? effectiveData.allocated_hours
    : 0;
  const safeAvailable = Number.isFinite(effectiveData.available_hours)
    ? effectiveData.available_hours
    : 0;
  const safeUtilization = Number.isFinite(effectiveData.utilization_percent)
  ...
  const tooltipContent = effectiveData.is_time_off ? (
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', minWidth: 200 }}>
      ...
      <span>Range Total: {safeRangeTotal.toFixed(1)}h</span>
      <span>Daily: {safeDailyAllocated.toFixed(1)}h</span>
      <span>Available: {safeAvailable.toFixed(1)}h</span>
      <span>Utilization: {safeUtilization.toFixed(0)}%</span>
      {/* <span>
          ...
          {isInteractive && (
            <div
              style={{
                ...
                border: `1px solid ${themeWiseColor('#fff', '#0B1221', themeMode)}`,
              }}
            />
          )}
        </div>
