            ...
            {visibleColumns.map((column, colIndex) => {
              ...
              const colStyle: React.CSSProperties = {
                ...
                ...(column.isSticky && {
                  ...
                  display: 'flex',
                  alignItems: 'center',
                  backgroundColor: isDarkMode ? '#141f2d' : '#ffffff',
                }),
              };
...
import { evt_project_task_list_visit } from '@/shared/proteccio-analytics-events';
...
const TaskListV2Section: React.FC = () => {
  ...
  const columnReorderStorageKey = urlProjectId
    ? `proteccio.taskList.columnOrder.${urlProjectId}`
    : null;
  ...
  const [columnWidths, setColumnWidths] = useState<Record<string, string>>(() => {
    ...
    try {
      const stored = localStorage.getItem(`proteccio.taskList.columnWidths.${urlProjectId}`);
      if (!stored) return {};
    ...
    if (urlProjectId && Object.keys(columnWidths).length > 0) {
      try {
        localStorage.setItem(
          `proteccio.taskList.columnWidths.${urlProjectId}`,
    ...
    const tagStickyWrappers = () => {
      container.querySelectorAll<HTMLElement>('*').forEach(el => {
        ...
        if (child && (child.classList.contains('mt-2') || child.querySelector('[data-group-header]'))) {
          ...
          // in both light (#ffffff) and dark (#0B1221) modes.
        ...
        >
          <div
            ...
            style={{
              ...
              backgroundColor: isDarkMode ? '#141f2d' : '#f9fafb',
            }}
          >
            <div
              ...
              {visibleColumns.map((column, index) => {
                ...
                const columnStyle: ColumnStyle = {
                  ...
                  ...(column.isSticky && {
                    ...
                    left: leftPosition,
                    zIndex: 15,
                    backgroundColor: isDarkMode ? '#141f2d' : '#f9fafb', // custom dark header : bg-gray-50
