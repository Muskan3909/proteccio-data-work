...
import { evt_account_setup_template_complete } from '@/shared/proteccio-analytics-events';
...
export const ProjectStep: React.FC<Props> = ({ onEnter, styles, isDarkMode = false, token }) => {
  ...
  const fetchTemplates = async () => {
    try {
      ...
      const templatesResponse = await projectTemplatesApiService.getProteccioTemplates();
  ...
  return (
    <div className="w-full project-step">
      ...
      <div className="mb-8">
        <Card
          className={`project-start-card hover:shadow-md transition-all duration-200 ${
