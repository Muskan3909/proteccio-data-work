          ...
          {organizationTypeOptions.map(option => {
            ...
            return (
              <button
                ...
                className={`p-2 rounded border transition-all duration-200 text-left hover:shadow-sm ${isSelected ? 'survey-option-selected' : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600'}`}
                style={{
                  ...
                  borderColor: isSelected ? '#2ed573' : token?.colorBorder,
                }}
              >
                <div className="flex items-center space-x-2">
                  <div
                    className={`w-3 h-3 rounded-full border flex items-center justify-center ${isSelected ? 'survey-option-radio-selected' : 'border-gray-300 dark:border-gray-600'}`}
                  >
                    {isSelected && <div className="w-1.5 h-1.5 bg-white rounded-full"></div>}
                  </div>
                  <span className="text-base">{option.icon}</span>
                  <span
                    className={`font-medium text-xs ${isSelected ? 'survey-option-text-selected' : ''}`}
          ...
          {useCaseOptions.map(option => {
            ...
            return (
              ...
              >
                <div className="flex items-center space-x-2">
                  <div
                    className={`w-3 h-3 rounded border flex items-center justify-center ${isSelected ? 'survey-option-radio-selected' : 'border-gray-300 dark:border-gray-600'}`}
                  >
                    {isSelected && (
          ...
          {howHeardAboutOptions.map(option => {
            ...
            return (
              <button
                ...
                className={`p-2 rounded border transition-all duration-200 hover:shadow-sm ${isSelected ? 'survey-option-selected' : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600'}`}
                style={{
                  backgroundColor: isSelected
                    ? isDarkMode
                      ? '#1a2432'
