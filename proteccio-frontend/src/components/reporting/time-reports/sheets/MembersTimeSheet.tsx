  ...
  ({ onTotalsUpdate }, ref) => {
    ...
    const fetchChartData = async () => {
      try {
        ...
        const filtersAreLoading =
          loadingTeams || loadingCategories || loadingProjects || loadingMembers || loadingUtilization;
        ...
        if (res.done) {
          ...
          const responseData = res.body as unknown as
            | { filteredRows?: IRPTTimeMember[]; totals?: Partial<IRPTTimeTotals> }
            | IRPTTimeMember[];
          const filteredRows = Array.isArray(responseData)
            ? responseData
            : responseData?.filteredRows || [];
          setJsonData(filteredRows);

          const totalsRaw = Array.isArray(responseData) ? {} : responseData?.totals || {};
          const totals = {
    ...
    const exportChart = () => {
      if (chartRef.current) {
        ...
        tempCtx.fillStyle = themeMode === 'dark' ? '#131B27' : '#ffffff';
