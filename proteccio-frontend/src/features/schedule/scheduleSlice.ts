...
export const updateWorking = createAsyncThunk(
  ...
  async ({ workingDays, workingHours }: { workingDays: string[]; workingHours: number }) => {
    ...
    if (!response.done) {
      throw new Error(response.message || 'Failed to update schedule settings');
    }
    const data = response.body;
    return data;
  }
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(updateWorking.fulfilled, (state, action) => {
        state.workingDays = Array.isArray(action.payload.workingDays)
      ...
      .addCase(getWorking.fulfilled, (state, action) => {
        state.workingDays = Array.isArray(action.payload.body?.workingDays)
          ? action.payload.body.workingDays
          : [];
