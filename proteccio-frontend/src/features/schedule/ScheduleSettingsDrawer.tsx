...
const ScheduleSettingsDrawer: React.FC = () => {
  ...
  useEffect(() => {
    const safeWorkingDays = Array.isArray(workingDays) ? workingDays : [];

    if (safeWorkingDays.length > 0 || workingHours > 0) {
      form.setFieldsValue({ workingDays: safeWorkingDays, workingHours });
    }
  ...
  const handleFormSubmit = async (values: any) => {
    try {
      await dispatch(
        updateWorking({
          workingDays: Array.isArray(values.workingDays) ? values.workingDays : [],
