...
import { evt_paywall_hit } from '@/shared/proteccio-analytics-events';
...
const Navbar = () => {
  ...
  const currentRoleDefinition = useMemo(() => getRoleDefinition(currentRole), [currentRole]);
  const canInviteMembers = currentRoleDefinition.canInviteMembers;
  ...
  const navlinkItems = useMemo(() => {
    ...
    return navRoutesList
      ...
      .map((route, index) => {
        ...
        return {
          key: route.path.split('/').filter(Boolean)[1] || route.name,
          disabled: false,
          label: shouldDisable ? (
  ...
  const currentRoute = useMemo(() => {
    const afterProteccioString = location.pathname.split('/proteccio/')[1];
    const pathKey = afterProteccioString?.split('/')[0];
    return pathKey ?? 'home';
  }, [location.pathname]);
  ...
  const handleMenuClick = useCallback(
    (menuInfo: { key: string }) => {
      ...
      const clickedRoute = navRoutesList.find(r => {
        const routeKey = r.path.split('/').filter(Boolean)[1] || r.name;
  ...
  return (
    <Col
      className="proteccio-navbar"
      style={{
        width: '100%',
        display: 'flex',
        flexDirection: 'column',
        paddingInline: isDesktop ? 44 : 24,
        height: 68,
        gap: 0,
        alignItems: 'center',
