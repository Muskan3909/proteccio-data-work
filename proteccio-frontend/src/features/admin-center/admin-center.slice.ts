...
// TODO(pre-existing, not part of the Proteccio rebrand): team-members-settings.tsx and
// project-view-members.tsx dispatch fetchBillingInfo(), but no such thunk/endpoint existed
// in this slice before the rebrand either. This no-op stub only exists so the app builds;
// wire it up to the real billing endpoint (or remove the dispatch calls) when convenient.
...
const adminCenterSlice = createSlice({
  ...
  reducers: {
    toggleRedeemCodeDrawer: state => {
      state.isRedeemCodeDrawerOpen        ? (state.isRedeemCodeDrawerOpen = false)
