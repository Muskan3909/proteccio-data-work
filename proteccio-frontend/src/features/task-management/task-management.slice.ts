...
export const fetchTasksV3 = createAsyncThunk(
  ...
  async (projectId: string, { rejectWithValue, getState }) => {
    try {
      ...
      const taskRows = Array.isArray(response.body?.allTasks) ? response.body.allTasks : [];
      const tasks: Task[] = taskRows.map((task: any) => normalizeTask(task));
      ...
      return {
        ...
        groups: Array.isArray(response.body?.groups) ? response.body.groups : [],
...
export const fetchTaskListColumns = createAsyncThunk(
  ...
  async (projectId: string, { dispatch }) => {
    ...
    return {
      standard: Array.isArray(standardColumns.body) ? standardColumns.body : [],
