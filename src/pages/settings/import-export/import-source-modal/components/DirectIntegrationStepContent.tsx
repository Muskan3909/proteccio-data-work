...
interface DirectIntegrationStepContentProps {
  ...
  ProteccioFieldOptions: Array<{ value: string; label: string }>;
...
export const DirectIntegrationStepContent: React.FC<DirectIntegrationStepContentProps> = props => {
  ...
  const ProteccioFieldLabelByValue = React.useMemo(() => {
    return new Map(ProteccioFieldOptions.map(option => [option.value, option.label]));
  }, [ProteccioFieldOptions]);
  ...
  if (step === 1) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center' }}>
        <div style={directContainerStyle}>
          <Typography.Title level={3} style={{ marginBottom: 8 }}>
            {t('importStep.setupProjectTitle', { defaultValue: 'Set up a project in Proteccio' })}
          </Typography.Title>
          <Typography.Paragraph style={{ color: themeToken.colorTextSecondary }}>
  ...
  if (step === 2) {
    if (reviewSubScreen === 'main') {
      const reviewCards = [
        ...
        },
        {
          ...
          description: t('importStep.importMembersDesc', { defaultValue: 'Brings collaborators into the Proteccio project' }),
          iconBg: '#0f9d58',
                    ...
                    : lowerKey === 'asana'
                      ? [
                        ...
                        t('importStep.limitationsAsanaLikes', {
                          defaultValue: 'Likes are imported as field values; reactions do not create social activity in Proteccio.',
                        }),
                      ...
                      : [
                        ...
                        t('importStep.limitationsGenericUsers', {
                          defaultValue: 'Unmatched users remain unresolved until they are added and mapped in Proteccio.',
                        }),
          ...
          >
            ...
            <Typography.Paragraph style={{ marginBottom: 16, color: themeToken.colorTextSecondary }}>
              {t(
                ...
                `How we've mapped your ${source.label} data to Proteccio. More about project hierarchy in Proteccio.`
              )}
            </Typography.Paragraph>
    ...
    if (reviewSubScreen === 'fieldMapping') {
      ...
      const fieldMappingDescription = t(
        ...
        "We've automatically mapped your {{source}} data into system and custom fields in Proteccio. You can customize some fields that have other compatible field types. More about field mapping.",
        { source: source.label || 'source' }
      );
      const panelBg = themeToken.colorBgContainer;
      const panelBorder = themeToken.colorBorder;
      ...
      const visibleRows = fieldMappingRows
        ...
        .filter(({ row }) => {
          ...
          const targetLabel =
            ProteccioFieldLabelByValue.get(row.target_field) ||
          ...
          >
            ...
            >
              ...
              >
                ...
                <span>{t('importStep.ProteccioField', 'Proteccio field')}</span>
              ...
              {fieldMappingRows.length === 0 ? (
                ...
                >
                  {visibleRows.map(({ row, index: rowIndex }) => {
                    const hasKnownLabel = ProteccioFieldLabelByValue.has(row.target_field);
                    const isCustomFieldCandidate =
                      !!row.target_field && !ProteccioFieldLabelByValue.has(row.target_field);
                    ...
                    const systemOptions = ProteccioFieldOptions.map(option => ({
                      value: option.value,
                      label: option.label,
                    ...
                    const options = [
                      ...(selectedOption.length
                        ...
                        : []),
                      {
                        label: t('importStep.ProteccioFields', {
                          defaultValue: 'Proteccio fields',
                        }),
