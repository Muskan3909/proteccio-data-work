...
export const CsvReviewStepContent: React.FC<CsvReviewStepContentProps> = ({
  ...
  const limitationItems = React.useMemo(() => {
    ...
    return [
      ...
      t('importStep.limitationsCsvUsers', {
        defaultValue: 'User references without valid emails stay unassigned until users are matched in Proteccio.',
      }),
    ];
  }, [providerKey, t]);
  ...
  const handleDownloadConfig = (e: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    a.href = url;
    a.download = 'Proteccio-import-config.json';
    a.click();
    ...
    <div style={{ width: '100%' }}>
      ...
      <Typography.Paragraph style={{ color: themeToken.colorTextSecondary, marginBottom: 24 }}>
        {t('importStep.reviewSpaceDetailsHelp', {
          defaultValue:
            "We're ready to import your team's data. Here's a summary of what will be imported into Proteccio.",
        })}
          ...
          <div>
            ...
            <div style={{ color: themeToken.colorTextSecondary, fontSize: 15 }}>
              {t('importStep.reviewProjectCardDescription', {
                defaultValue: 'A new Proteccio project will be created for this import.',
              })}
            </div>
          </div>
        </div>
          ...
          <div>
            ...
            <div style={{ color: themeToken.colorTextSecondary, fontSize: 15 }}>
              {t('importStep.reviewFieldsCardDescription', {
                defaultValue: '{{mapped}} columns will be mapped to existing Proteccio fields.',
                mapped: mappedFields,
              })}
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{ color: themeToken.colorTextSecondary, fontSize: 15 }}>
              {t('importStep.reviewWorkTypesCardDescription', {
                defaultValue:
                  'If values are not mapped to Proteccio statuses, all tasks are mapped to Task (level 0) by default.',
              })}
            </div>
          </div>
        </div>
