...
interface TemplateDrawerProps {
  ...
  selectedTemplateType: (type: 'Proteccio' | 'custom') => void;
}
...
const TemplateDrawer: React.FC<TemplateDrawerProps> = ({
  ...
  selectedTemplateType = (type: 'Proteccio' | 'custom') => {},
}) => {
  ...
  const [templates, setTemplates] = useState<IProteccioTemplate[]>([]);
  ...
  const getTemplates = async () => {
    try {
      ...
      const res = await projectTemplatesApiService.getProteccioTemplates();
      if (res.done) {
  ...
  const handleTabChange = (key: string) => {
    if (key === '1') {
      getTemplates();
      selectedTemplateType('Proteccio');
      trackMixpanelEvent(evt_project_import_from_template_click, { template_tab: 'Proteccio' });
    } else {
      getCustomTemplates();
      selectedTemplateType('custom');
