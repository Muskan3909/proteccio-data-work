...
const GranttMembersTable = React.memo(
  ({
    ...
    const scheduleDate = useAppSelector(state => state.scheduleReducer.date);
    const scheduleType = useAppSelector(state => state.scheduleReducer.type);
    ...
    const navigateToProject = useCallback(
      (projectId: string) => {
        navigate(`/Proteccio/projects/${projectId}?tab=tasks-list&pinned_tab=tasks-list`);
      },
      [navigate]
    );
    ...
    const handleToggleDrawer = useCallback((memberId: string) => {
      const selectedDay = dayjs(scheduleDate);
      const rangeStart =
        scheduleType === 'month'
          ? selectedDay.startOf('month')
          : selectedDay.startOf('week').add(1, 'day');
      const rangeEnd =
        scheduleType === 'month' ? selectedDay.endOf('month') : rangeStart.add(13, 'day');
    ...
    }, [dispatch, scheduleDate, scheduleType]);
                    ...
                    <Flex vertical gap={4} style={{ flex: 1 }}>
                      <Flex align="center" gap={8}>
                        <Button
                          ...
                          onClick={() => handleToggleDrawer(memberId)}
                        >
                          {member.name}
                        </Button>
                        {/* {(() => {
