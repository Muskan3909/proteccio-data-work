...
# 5. Installs and starts Proteccio
# ============================================================================
...
print_header() {
    ...
    echo -e "${NC}"
    echo -e "${CYAN}╔════════════════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║           ${BLUE}Proteccio Quick Setup - Automated Installation${CYAN}              ║${NC}"
    echo -e "${CYAN}╚════════════════════════════════════════════════════════════════════════╝${NC}"
    echo ""
}
...
echo "  5. Install and start Proteccio"
echo ""
...
if [[ "$build_images" =~ ^[Yy]$ ]]; then
    ...
    if [ -n "$docker_username" ]; then
        ...
        if [ -f "$SCRIPT_DIR/docker-compose.yaml" ]; then
            sed -i.bak "s|image: .*/Proteccio-backend:latest|image: $docker_username/Proteccio-backend:latest|" "$SCRIPT_DIR/docker-compose.yaml"
            sed -i.bak "s|image: .*/Proteccio-frontend:latest|image: $docker_username/Proteccio-frontend:latest|" "$SCRIPT_DIR/docker-compose.yaml"
    ...
    echo "  - Answer 'no' to use default images (chamikajaycey/Proteccio-*)"
    echo ""
...
if [[ "$domain" == "localhost" ]]; then
    echo -e "  ${GREEN}Access Proteccio at:${NC} ${BLUE}https://localhost${NC}"
    echo ""
    ...
    echo -e "  ${GREEN}Access Proteccio at:${NC} ${BLUE}https://$domain${NC}"
    echo ""
