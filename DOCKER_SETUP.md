# Docker Setup Guide - Production-Ready Proteccio
...
- Non-root user (`Proteccio`) for security
...
Commands:
  install          Install Proteccio (auto-generates secrets)
...
ls -la Proteccio-backend/database/sql/
```
...
1. **Backup your data**:
   ```bash
   docker compose exec db pg_dump -U postgres Proteccio_db > backup.sql
   ```
