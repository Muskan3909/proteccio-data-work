...
# ============================================================================
# Proteccio Local Management Script
# ============================================================================
# This script provides an interactive menu for managing local Proteccio deployment
...
print_header() {
    ...
    echo -e "${NC}"
    echo -e "${CYAN}╔════════════════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║           ${BLUE}Proteccio Self-Hosted Management Console${CYAN}                    ║${NC}"
    echo -e "${CYAN}╚════════════════════════════════════════════════════════════════════════╝${NC}"
    echo ""
}
...
setup_self_signed_ssl() {
    ...
    if command -v openssl &> /dev/null; then
        openssl req -x509 -nodes -days 365 -newkey rsa:2048 \
            ...
            -subj "/C=US/ST=State/L=City/O=Proteccio/CN=localhost" \
            &> /dev/null
...
setup_letsencrypt_ssl() {
    ...
    if [ ${PIPESTATUS[0]} -eq 0 ]; then
        ...
        cp "$SCRIPT_DIR/nginx/conf.d/Proteccio.conf" "$SCRIPT_DIR/nginx/conf.d/Proteccio.conf.bak"
        ...
        sed -i.tmp \
            ...
            "$SCRIPT_DIR/nginx/conf.d/Proteccio.conf"
        
        rm -f "$SCRIPT_DIR/nginx/conf.d/Proteccio.conf.tmp"
        ...
        print_info "Backup saved: nginx/conf.d/Proteccio.conf.bak"
        return 0
    else
...
start_services() {
    print_header
    echo -e "${BLUE}Starting Proteccio Docker Environment...${NC}"
    echo ""
    ...
    print_success "Proteccio started!"
    echo ""
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "  ${GREEN}Access Proteccio at:${NC} ${VITE_API_URL:-https://localhost}"
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
...
stop_services() {
    print_header
    echo -e "${BLUE}Stopping Proteccio Services${NC}"
    echo ""
...
service_status() {
    ...
    # Check each service
    docker ps --filter "name=Proteccio-" --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}" | grep -E "Proteccio-|NAMES"
}
...
backup_data() {
    print_header
    echo -e "${BLUE}Backup Proteccio Data${NC}"
    echo ""
    ...
    $compose_cmd exec -T postgres pg_dump -U "${DB_USER:-postgres}" "${DB_NAME:-Proteccio_db}" > "$backup_dir/database.sql"
    ...
    if [[ "${DEPLOYMENT_MODE:-express}" == "express" ]]; then
        ...
        docker run --rm -v Proteccio_redis_data:/data -v "$backup_dir:/backup" alpine tar czf "/backup/redis_data.tar.gz" -C /data . 2>/dev/null
        print_success "Redis data backed up"

        # Backup MinIO data
        print_info "[3/4] Backing up MinIO storage..."
        docker run --rm -v Proteccio_minio_data:/data -v "$backup_dir:/backup" alpine tar czf "/backup/minio_data.tar.gz" -C /data . 2>/dev/null
    ...
    tar -czf "$BACKUP_DIR/Proteccio_backup_$timestamp.tar.gz" -C "$BACKUP_DIR" "backup_$timestamp"
    rm -rf "$backup_dir"

    local backup_file="$BACKUP_DIR/Proteccio_backup_$timestamp.tar.gz"
    ...
    find "$BACKUP_DIR" -name "Proteccio_backup_*.tar.gz" -mtime +$retention_days -delete
}

restore_data() {
    print_header
    echo -e "${BLUE}Restore Proteccio Data${NC}"
    echo ""
    ...
    local backups=($(ls -t "$BACKUP_DIR"/Proteccio_backup_*.tar.gz 2>/dev/null))
    ...
    for backup in "${backups[@]}"; do
        ...
        local date_str=$(echo "$filename" | sed 's/Proteccio_backup_\(.*\)\.tar\.gz/\1/' | sed 's/_/ /; s/_/:/')
    ...
    if [ -f "$temp_dir/$backup_content/database.sql" ]; then
        $compose_cmd exec -T postgres psql -U "${DB_USER:-postgres}" -d "${DB_NAME:-Proteccio_db}" < "$temp_dir/$backup_content/database.sql" 2>/dev/null
    ...
    if [ -f "$temp_dir/$backup_content/redis_data.tar.gz" ]; then
        ...
        docker run --rm -v Proteccio_redis_data:/data -v "$temp_dir/$backup_content:/backup" alpine sh -c "rm -rf /data/* && tar xzf /backup/redis_data.tar.gz -C /data"
    ...
    if [ -f "$temp_dir/$backup_content/minio_data.tar.gz" ]; then
        ...
        docker run --rm -v Proteccio_minio_data:/data -v "$temp_dir/$backup_content:/backup" alpine sh -c "rm -rf /data/* && tar xzf /backup/minio_data.tar.gz -C /data"
...
auto_configure_env() {
    ...
    if [[ "$REDIS_PASSWORD" == "Proteccio_redis_pass" ]]; then
...
build_images() {
    ...
    echo ""
    docker build \
        -f "$SCRIPT_DIR/Proteccio-backend/Dockerfile" \
        -t "${docker_username}/Proteccio-backend:${image_version}" \
        "$SCRIPT_DIR/Proteccio-backend"
    ...
    echo ""
    docker build \
        -f "$SCRIPT_DIR/Proteccio-frontend/Dockerfile" \
        -t "${docker_username}/Proteccio-frontend:${image_version}" \
        "$SCRIPT_DIR/Proteccio-frontend"
    ...
    sed -i.bak "s|image: Proteccio-backend:.*|image: ${docker_username}/Proteccio-backend:${image_version}|" "$DOCKER_COMPOSE_FILE"

    # Update frontend image
    sed -i.bak "s|image: Proteccio-frontend:.*|image: ${docker_username}/Proteccio-frontend:${image_version}|" "$DOCKER_COMPOSE_FILE"
    ...
    echo ""
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "  ${GREEN}Backend Image:${NC} ${docker_username}/Proteccio-backend:${image_version}"
    echo -e "  ${GREEN}Frontend Image:${NC} ${docker_username}/Proteccio-frontend:${image_version}"
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    ...
    if [[ "$push_choice" =~ ^[Yy]$ ]]; then
        ...
        docker push "${docker_username}/Proteccio-backend:${image_version}"
        ...
        docker push "${docker_username}/Proteccio-frontend:${image_version}"
        ...
        echo ""
        echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
        echo -e "  ${GREEN}Backend:${NC} https://hub.docker.com/r/${docker_username}/Proteccio-backend"
        echo -e "  ${GREEN}Frontend:${NC} https://hub.docker.com/r/${docker_username}/Proteccio-frontend"
        echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    else
...
push_images() {
    ...
    if ! docker images | grep -q "${docker_username}/Proteccio-backend"; then
        print_error "Backend image not found locally!"
        print_info "Please build images first: ./manage.sh build"
        return 1
    fi

    if ! docker images | grep -q "${docker_username}/Proteccio-frontend"; then
...
install_Proteccio() {
    print_header
    echo -e "${BLUE}Installing Proteccio${NC}"
    echo ""
    ...
    echo "  - For production: enter your domain (e.g., Proteccio.example.com)"
    echo ""
...
upgrade_Proteccio() {
    print_header
    echo -e "${BLUE}Upgrade Proteccio${NC}"
    echo ""
    ...
    case $ssl_choice in
        ...
        4)
            ...
            if [ ! -z "${DOMAIN}" ] && [ -d "/etc/letsencrypt/live/${DOMAIN}" ]; then
                ...
                docker run --rm -v Proteccio_certbot_certs:/etc/letsencrypt alpine/openssl x509 -in "/etc/letsencrypt/live/${DOMAIN}/fullchain.pem" -text -noout | grep -E "(Subject:|Issuer:|Not Before|Not After)"
            fi
            read -p "Press Enter to continue..."
            ;;
        0)
...
if [ $# -gt 0 ]; then
    case "$1" in
        ...
        build-push)
            ...
            ;;
        help|--help|-h)
            echo "Proteccio Management Script"
            echo ""
            echo "Usage: $0 [command]"
            echo ""
            echo "Commands:"
            echo "  install          Install Proteccio (auto-generates secrets)"
