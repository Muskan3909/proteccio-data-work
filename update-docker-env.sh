...
mkdir -p Proteccio-frontend
cat > Proteccio-frontend/.env.development << EOL
# API Connection
VITE_API_URL=http://localhost:3000
VITE_SOCKET_URL=ws://localhost:3000

# Application Environment
VITE_APP_TITLE=Proteccio
...
# Session ID
VITE_Proteccio_SESSION_ID=Proteccio-session-id
EOL

# Create frontend .env.production file
cat > Proteccio-frontend/.env.production << EOL
# API Connection
...
mkdir -p Proteccio-backend
cat > Proteccio-backend/.env << EOL
# Server
NODE_ENV=production
PORT=3000
SESSION_NAME=Proteccio.sid
...
echo "- Proteccio-frontend/.env.development (development)"
echo "- Proteccio-frontend/.env.production (production build)"
echo "- Proteccio-backend/.env"
echo
